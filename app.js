(function () {
  ...
  // ---- pattern insights (Step 5) ----
  // Pure correlation/trend logic lives in insights-utils.js; rendering stays here.
  const { INPUT_TAGS, OUTCOMES, findPatterns, bloatingClusters, weightTrend } = window.InsightsUtils;
  ...
  // ---- health logging (Phase C) ----
  // Pure exercise/vitamin/med folding lives in health-utils.js; rendering stays
  // here. Health inputs feed the insights engine as dynamic presence tags
  // (exercise:<id> / vit:<id> / med:<name>), built fresh in healthInputContext().
  const {
    EXERCISE_TYPES, KINDS: HEALTH_KINDS, exerciseLabel, exerciseEmoji, medKey,
    healthOn, exerciseTag, vitaminTag, medTag, summariseHealthDays, medCount,
  } = window.HealthUtils;
  ...
  async function renderDashboard() {
    ...
    await renderDashInsights(checkins);
  ...
  async function submitCheckin(e) {
    ...
    await HimaStore.addCheckin({ date: todayISO(), at: Date.now(), mood, energy, tags, food, foodTags, note });
  ...
  async function renderWeight() {
    ...
    const weights = await HimaStore.getWeights(); // ascending by `at`
  ...
  async function submitWeight(e) {
    ...
    await HimaStore.addWeight({ date: todayISO(), at: Date.now(), kg, feeling });
  ...
  async function onWeightListClick(e) {
    ...
    const row = e.target.closest(".checkin-row");
    if (!btn || !row) return;
    await HimaStore.deleteWeight(row.dataset.id);
    renderWeight();
  }

  // ============ HEALTH (Phase C) ============
  // Exercise (multi-select), vitamins (define-once + daily tick), and meds
  // (name + count). All optional; all feed insights as presence inputs. Wording
  // stays neutral — a plain record, never advice.
  async function renderHealth() {
    const today = todayISO();
    const [entries, vitamins] = await Promise.all([
      HimaStore.getHealth({ date: today }),
      HimaStore.getVitamins(),
    ]);
  ...
  async function submitExercise(e) {
    ...
    await HimaStore.addHealth({ date: todayISO(), at: Date.now(), kind: HEALTH_KINDS.exercise, types });
  ...
  async function onVitaminListClick(e) {
    ...
    if (toggle) {
      const today = todayISO();
      const entries = await HimaStore.getHealth({ date: today });
      const existing = entries.find((h) => h.kind === HEALTH_KINDS.vitamin && h.vitId === vitId);
      if (existing) await HimaStore.deleteHealth(existing.id);
      else await HimaStore.addHealth({ date: today, at: Date.now(), kind: HEALTH_KINDS.vitamin, vitId });
      renderHealth();
    } else if (del) {
      await HimaStore.deleteVitamin(vitId);
  ...
  async function submitVitaminDef(e) {
    ...
    await HimaStore.addVitamin({ name });
    $("vitamin-name").value = "";
    renderHealth();
  }
  async function submitMed(e) {
    ...
    await HimaStore.addHealth({ date: todayISO(), at: Date.now(), kind: HEALTH_KINDS.med, name, count });
    toast("Logged. 🌿");
    renderHealth();
  }
  ...
  async function onHealthTodayClick(e) {
    ...
    const row = e.target.closest(".checkin-row");
    if (!btn || !row) return;
    await HimaStore.deleteHealth(row.dataset.id);
    renderHealth();
  }
  ...
  // ============ PATTERN INSIGHTS (Step 5) ============
  // Reflect, never diagnose: every string here is a neutral observation of the
  // user's own co-occurrences. No advice, no goals, no red zones.
  async function renderInsights() {
    const checkins = await HimaStore.getCheckins();
    const weights = await HimaStore.getWeights();
    const { healthDays, inputTags } = await healthInsightsContext();
  ...
  async function init() {
    ...
    $("dash-insights").addEventListener("click", () => showView("insights"));
    ...
    // Health (Phase C): exercise multi-select chips, vitamin list, med logger.
